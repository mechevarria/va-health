...
def get_simplied_group_network(src, name, color_name):
    ...
    for k in groups.keys():
        # Mike requested the radius scale between 30 and 10
        size = (groups[k]['row_count'] - min_size) / \
            (max_size - min_size) * (30-10) + 10
        data.append([str(k), str(k)])
        nodes.append({'id': k, 'colorScale': group_colors[int(k)], "marker": {"radius": size}})

    # Create Nodes with edges
    for f, t in combs:
        if not set(groups[f]['row_indices']).isdisjoint(set(groups[t]['row_indices'])):
            data.append([f, t])
...
def get_normal_network(src, name, color_name):
  ...
  '''
    ...
    # get coloring values
    outcome_coloring = src.create_coloring(
        name=color_name, column_name=color_name)
