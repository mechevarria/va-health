...
def compute_kpis(src, group_id):
  ...
  vac_values = group_stat(src, grp, 'covid_vaccine_TotalSeriesCount')
  ...
  hr = group_stat(src, grp, 'covid_post_procedures_Hospitalization60d')
  hrp = sum(hr) / len(hr) * 100 
  kpis.append({"name": "Hospitilization Rate", "value":  f"{hrp:.1f}%"})

  in_per_vis = group_stat(src, grp, 'visits_count_proportion_Presumed In Person_period3')
  ...
  a1c = group_stat(src, grp, 'A1C_last_period4')
  a1c = sum(a1c) / len(a1c) 
